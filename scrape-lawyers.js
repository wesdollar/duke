...
async function getAllLayersData(page) {

    let file = './all-lawyers-hrefs.json'

    let lawyers = require(file)

    while (lawyers.length > 0) {

        let href =  'https://www.avvo.com' + lawyers[0].href

        // get one lawyer
        let result = await getLawyerData(page, href)

        lawyers.splice(0, 1)

        fs.writeFileSync(file, JSON.stringify(lawyers))
    }
}
